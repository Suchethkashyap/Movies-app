...
export default function MovieDetails() {
  ...
  function generateEmbeddedVideoURL(trailer: string) {
    if (!trailer || typeof trailer !== "string") {
      return "";
    }

    const videoIdMatch = trailer.match(
      /(?:\?v=|\/embed\/|\/\d+\/|\.be\/|\/embed\/|\/v\/|http:\/\/youtu.be\/|\/\d+\/|\/v=|^youtu.be\/|^www.youtube.com\/v\/|\/www.youtube.com\/v\/)([^#\&\?\n]*).*/i
    );
    ...
    <div>
      ...
      {movie.genres?.map((genre) => (
        <Link
          ...
          to={`/movies/filter?GenreIds=${genre.id}`}
        >
          {genre.name}
        </Link>
      ))}
